...
public class StudentDaoImpl implements StudentDao {
	...
	public List<StudentDTOO> getAllDetailsByStudentName(String username) throws StudentException {
		...
		try (Connection conn = DBUtil.provideConnection()){
			...
			PreparedStatement ps = conn.prepareStatement("select s.roll,s.username,s.name,s.address,s.mobile,c.courseid,c.cname,c.fees,c.duration,b.batchno,b.seats from student s,course c,batch b,student_course sc,student_batch sb where s.roll = sc.roll AND c.courseid = sc.courseid AND s.roll = sb.roll AND b.batchno = sb.batchno AND s.username = ?;");
			ps.setString(1, username);
