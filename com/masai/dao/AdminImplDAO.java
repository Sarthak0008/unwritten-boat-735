...
public class AdminImplDAO implements AdminDAO {
	...
	public String registerNewCourse(Course course) {
		...
		try (Connection conn = DBUtil.provideConnection()){
			...
			PreparedStatement ps = conn.prepareStatement("insert into course(cname,fees,duration) values(?,?,?)");
	...
	public String updateCourseDuration(Course course) {
		...
		PreparedStatement ps = conn.prepareStatement("update course set duration = ? where cname = ?");
	...
	public List<CourseDTO> getAllDetailsByCourseName(String cname) throws CourseException {
		...
		try (Connection conn = DBUtil.provideConnection()){
			...
			PreparedStatement ps = conn.prepareStatement("select c.courseid,c.cname,c.fees,c.duration,s.roll,s.username,s.name,s.address,s.mobile from course c,student s,student_course sc where c.courseid = sc.courseid AND s.roll = sc.roll AND cname = ?");
